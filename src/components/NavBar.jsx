...
export default function NavBar({ setLogin }) {
    ...
    return(
        <div className="h-[7svh] flex justify-end p-3 gap-3">
            {!isLoading && (user ? <div onClick={logout}>{user.username}</div> : <button onClick={()=>setLogin(true)} className="font-semibold px-3 py-4 flex items-center text-lg hover:text-zinc-400 duration-150">Login</button>)}
