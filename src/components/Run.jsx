        ...
        <div className="flex h-svh w-full justify-center items-center bg-transparent absolute z-20">
            <div className="border-2 border-zinc-700 w-[35%] bg-zinc-900 h-[85%] rounded-xl flex flex-col py-5 px-3 gap-5">
                <div className="flex justify-between px-3 items-center">
                    <button onClick={run} className="hover:text-emerald-400 text-emerald-200 duration-150 text-right px-2 "><span className="material-symbols-outlined text-2xl">play_arrow</span></button>
                            ...
                            <div key={index} className="px-5 py-1 rounded-xl flex flex-col">
                                {item.url &&<div className="flex justify-between">
                                    ...
                                    {!item.isLoading && (savedResponse[index]?.statusCode < 300 ) && <div className="flex items-center gap-3 w-[20%]">
                                        ...
                                        <p className="text-sm text-zinc-400 font-semibold">{savedResponse[index].statusCode}</p>
                                    </div>}
                                    ...
                                    {!item.isLoading && savedResponse[index] && (!savedResponse[index]?.ok) && <div className="flex items-center gap-3 w-[20%]">
                                        ...
                                        {(savedResponse[index]?.statusCode >= 300 )?<p className="text-sm text-zinc-400 font-semibold">{savedResponse[index].statusCode}</p>:<p className="text-sm text-zinc-400 font-semibold">Error</p>}
                                        </div>
                                    }
                                </div>
                                }
